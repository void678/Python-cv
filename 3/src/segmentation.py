...
TRAIN_RESULT_DIR = os.path.join(DATASET_DIR, 'train', 'result')
...
if __name__ == "__main__":
    ...
    while id <= 1000:
        ...
        img_number = str(id)
        # 读取相应编号原图和前景图
        img = cv2.imread(os.path.join(IMGS_DIR, img_number + ".png"))
        img_gt = cv2.imread(os.path.join(GT_DIR, img_number + ".png"))
        # 保存到dateset/train/imgs和dateset/train/gt
        cv2.imwrite(os.path.join(TRAIN_IMGS_DIR, img_number + "_origin.png"), img)
        cv2.imwrite(os.path.join(TRAIN_GT_DIR, img_number + "_gt.png"), img_gt)
        id += 5
    ...
    while id <= 1000:
        ...
        img = cv2.imread(os.path.join(IMGS_DIR, img_number + ".png"))
        img_gt = cv2.imread(os.path.join(GT_DIR, img_number + ".png"))
        # 保存到dateset/test/imgs和dateset/test/gt
        cv2.imwrite(os.path.join(TEST_IMGS_DIR, img_number + "_origin.png"), img)
        cv2.imwrite(os.path.join(TEST_GT_DIR, img_number + "_gt.png"), img_gt)
        id += 100
    ...
    while index <= 1000:
        ...
        if img is None or img_gt is None:
            print(f"Error: {img_number} origin or gt not found!")
            index += 5
            continue
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        ...
        print(f"  IOU: {IOU}")
        # 按序号保存图片
        cv2.imwrite(os.path.join(TRAIN_RESULT_DIR, img_number + '_result.png'), img)
        cv2.imwrite(os.path.join(TRAIN_MASK_DIR, img_number + '_mask.png'), img2)
        cv2.waitKey(0)
    ...
    while index <= 1000:
        id = index
        print(f"Processing test image {id}.png ...")
        img_number = str(id)
        img = cv2.imread(os.path.join(TEST_IMGS_DIR, img_number + "_origin.png"))
        img_gt = cv2.imread(os.path.join(TEST_GT_DIR, img_number + "_gt.png"))
        if img is None or img_gt is None:
            print(f"Error: {img_number} test origin or gt not found!")
            index += 100
            continue
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        height, width, channel = img.shape
        ...
        IOU, img2 = cal_IOU(img_gt, res, width, height, areaset)
        print(f"  IOU: {IOU}")
        cv2.imwrite(os.path.join(TEST_RESULT_DIR, img_number + '_result.png'), img)
        cv2.imwrite(os.path.join(TEST_MASK_DIR, img_number + '_mask.png'), img2)
        index += 100
