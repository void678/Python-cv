...
class SeamCarving():
    def getDynamicEnergyMap(self, img, mask):
        ...
        dynamicEnergyMap = np.zeros((height + 2, width + 2), dtype=np.float32)  
        routeMap = np.ones((height, width), dtype=np.int32) * -1
        for hIndex in range(height):  # 从上到下逐行处理
            M = np.zeros((width, 3), dtype=np.float32)  
            # 采用 Forward Seam Removing 机制进行计算
    ...
    def removeSeam(self, img, seamIndex):
        ...
        if len(img.shape) == 3:
            newImg = np.zeros((height, width-1, img.shape[2]), dtype=img.dtype)
        else:
            newImg = np.zeros((height, width-1), dtype=img.dtype)
            
        for i in range(height):
            col_idx = seamIndex[i]
            newImg[i, :col_idx] = img[i, :col_idx]
            newImg[i, col_idx:] = img[i, col_idx+1:]
...
if __name__ == '__main__':
    ...
    # 设置图片和掩码的路径
    base_dir = 'D:/HW_2'  # 基础目录
    imgs_dir = f'{base_dir}/imgs'  # 图片目录
    gt_dir = f'{base_dir}/gt'      # 掩码目录
    ...
    for id in ids:
        try:
            ...
            img_path = f'{imgs_dir}/{id}.png'
            mask_path = f'{gt_dir}/{id}.png'
            ...
            imgDict[id] = dict()
            imgDict[id]['img'] = np.asarray(img).copy().astype(np.int32)
            
            # 读取对应Mask
            img = Image.open(mask_path)
            imgArray = np.asarray(img).copy().astype(np.float32)
            # 对Mask进行二值化
            imgArray[imgArray > 0] = 1
            imgDict[id]['mask'] = np.asarray(imgArray[:, :, 0])
            print(f"Successfully loaded image and mask for id {id}")
    ...
    if not imgDict:
        print("No images were loaded successfully. Please check the paths and file names.")
        exit(1)
